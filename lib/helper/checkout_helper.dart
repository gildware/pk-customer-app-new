...
class CheckoutHelper {
  ...
  /// Admin-enabled digital gateways from config API (`payment_gateways`).
  static List<DigitalPaymentMethod> enabledDigitalPaymentGateways() {
    final gateways = configModel.content?.paymentMethodList ?? [];
  ...
  /// Cart-level additional charges from API (Business setup → Additional charges).
  static bool requiresBookingUpfrontPayment() {
    final value = configModel.content?.requireBookingUpfrontPayment;
    // Match backend default (enabled) when config is missing from cached API response.
    if (value == null) {
      return true;
    }
    return value == 1;
  ...
  /// Show confirmation vs full payment on cart checkout (not repeat / custom post).
  static bool showBookingPaymentAmountOptions({required String fromPage, required bool isRepeatBooking}) {
  ...
  static double bookingConfirmationAmount({required double fullAmount}) {
    final units = bookingConfirmationUnits();
    if (units <= 0) {
      return 0;
    }
    final confirmation = bookingConfirmationAmountPerService() * units;
    return confirmation > fullAmount ? fullAmount : confirmation;
  }

  static double payableAmountForCheckout({required double fullAmount, String? paymentAmountType}) {
    if (!requiresBookingUpfrontPayment()) {
      return fullAmount;
    }
    if (paymentAmountType == 'confirmation') {
      return bookingConfirmationAmount(fullAmount: fullAmount);
    }
    return fullAmount;
  }

  static double getAdditionalChargeTotal() {
    if (Get.isRegistered<CartController>()) {
      final cartController = Get.find<CartController>();
      if (cartController.additionalChargeTotal > 0) {
        return cartController.additionalChargeTotal;
      }
    }
    return getAdditionalCharge();
  }

  static List<CartAdditionalChargeLine> getAdditionalChargeLines() {
    if (Get.isRegistered<CartController>()) {
      return Get.find<CartController>().additionalChargeLines;
    }
    return [];
  }
  ...
  static AddressModel? selectedAddressModel ({AddressModel? selectedAddress, AddressModel? pickedAddress, ServiceLocationType selectedLocationType = ServiceLocationType.customer}){
    ...
    if(selectedAddress !=null && (selectedAddress.zoneId == pickedAddress?.zoneId)){
      addressModel = AddressModel(
        ...
        availableServiceCountInZone: selectedAddress.availableServiceCountInZone,
