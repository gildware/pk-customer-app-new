...
class Service {
  ...
  /// Lowest price from zone-wise variations, campaign variations, or [defaultPrice].
  num resolveLowestPrice({bool fromCampaign = false}) {
    if (fromCampaign) {
      if (variations == null || variations!.isEmpty) {
        return 0;
      }
      num lowest = variations!.first.price ?? 0;
    ...
    if (variationsAppFormat?.hasZoneWiseVariations ?? false) {
      return variationsAppFormat!.lowestZonePrice;
    }
    return _lowestFromRawVariations();
  }

  bool get hasBookableVariations {
    if (variationsAppFormat?.hasZoneWiseVariations ?? false) {
      return true;
    }
  ...
  List<ZoneWiseVariations> get bookableVariations {
    if (variationsAppFormat?.hasZoneWiseVariations ?? false) {
      return variationsAppFormat!.zoneWiseVariations!;
    }
    if (variations == null || variations!.isEmpty) {
      return [];
    }
    final seen = <String>{};
    final list = <ZoneWiseVariations>[];
    for (final variation in variations!) {
      final key = variation.variantKey ?? '';
      if (key.isEmpty || seen.contains(key)) {
        continue;
      }
      seen.add(key);
      list.add(ZoneWiseVariations(
        variantKey: key,
        variantName: variation.variant ?? key.replaceAll('-', ' '),
  ...
  num _lowestFromRawVariations() {
    final items = bookableVariations;
    if (items.isEmpty) {
      return 0;
    }
    num lowest = items.first.price ?? 0;
...
class VariationsAppFormat {
  ...
  bool get hasZoneWiseVariations =>
      zoneWiseVariations != null && zoneWiseVariations!.isNotEmpty;

  num get lowestZonePrice {
    if (!hasZoneWiseVariations) {
      return defaultPrice ?? 0;
    }
    num lowest = zoneWiseVariations!.first.price ?? defaultPrice ?? 0;
