          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            CustomDateRangePicker(
              onTimeChanged: (DateTimeRange? dateRange){
                if(dateRange != null && scheduleController.pickedDailyRepeatTime != null) {
                  DateTime startDate = dateRange.start;
                  DateTime now = DateTime.now();
                  if(startDate.year == now.year && startDate.month == now.month && startDate.day == now.day) {
                    TimeOfDay pickedTime = scheduleController.pickedDailyRepeatTime!;
                    TimeOfDay nowTime = TimeOfDay.now();
                    if(pickedTime.hour < nowTime.hour || (pickedTime.hour == nowTime.hour && pickedTime.minute < nowTime.minute)) {
                      customSnackBar('selected_time_cannot_earlier'.tr);
                      scheduleController.updatePickedDailyRepeatTime = null;
                    }
                  }
                }
            ...
            CustomTimePicker(
              onTimeChanged: (TimeOfDay? time) {
                if(time != null && scheduleController.pickedDailyRepeatBookingDateRange != null) {
                  DateTime startDate = scheduleController.pickedDailyRepeatBookingDateRange!.start;
                  DateTime now = DateTime.now();
                  if(startDate.year == now.year && startDate.month == now.month && startDate.day == now.day) {
                    TimeOfDay nowTime = TimeOfDay.now();
                    if(time.hour < nowTime.hour || (time.hour == nowTime.hour && time.minute < nowTime.minute)) {
                      customSnackBar('selected_time_cannot_earlier'.tr);
                      return;
                    }
                  }
